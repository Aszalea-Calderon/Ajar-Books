...
| ☐    | Extract the status dropdown and format-picker pills out of the book detail page into their own reusable components — prep work for the two items below, and for restyling either one later without touching page logic                                                                                                                                                                                          |
| ☐    | Restyle the status control (Want to Read / Currently Reading / Finished / DNF) to match the rest of the platform's control styling (pills/buttons elsewhere) instead of a plain `<select>`                                                                                                                                                                                                                      |
| ☐    | Replace the always-visible format-toggle pills with a single "Start Reading" action: clicking it opens a modal to pick format + starting total, and always sets status straight to Currently Reading (even from Want to Read) — the one action that begins tracking. Once reading has started, a separate "Change format" action (not the original pills) stays available so format isn't locked in permanently |
